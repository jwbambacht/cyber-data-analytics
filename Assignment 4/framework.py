...
def main(config_file="parameters.ini"):
    ...
    def train(epoch):
        ...
        for batch_idx, ((bon_x, bon_y), (mal_x, mal_y)) in enumerate(
            ...
            with open("benign.dat", 'a+') as fh:
                fh.write(",".join(str(bon_x[x][y]) \
                    for x in range(len(bon_x)) \
                    for y in range(len(bon_x[0]))))
                fh.write("\n")    

            with open("malicious.dat", 'a+') as fh:
                fh.write(",".join(str(mal_x[x][y]) \
                    for x in range(len(mal_x)) \
                    for y in range(len(mal_x[0]))))
                fh.write("\n")   
            ...
            with open("malicious_modified.dat", 'a+') as fh:
                fh.write(",".join(str(mal_x[x][y]) \
                    for x in range(len(mal_x)) \
                    for y in range(len(mal_x[0]))))
                fh.write("\n")    
...
#             print("PREDICTED",correct, predicted.data, y.data)
    ...
    def check_one_category(category="benign", dset_type='test', is_evade=False,
        ...
        for batch_idx, (x, y) in enumerate(dataloader):
            ...
            pred_x = predicted.data.cpu().numpy()[0][0]
            y_val = y.data.view_as(predicted.data).cpu().numpy()[0]
            
            if pred_x == y_val and pred_x == 1:
            	tp += 1
            elif pred_x == y_val and pred_x == 0:
            	tn += 1
            elif pred_x != y_val and pred_x == 1:
            	fn += 1
            elif pred_x != y_val and pred_x == 0:
            	fp += 1
        ...
        print("TP:",tp,"FP:", fp, "FN", fn, "TN:", tn,"TOTAL:", total_cm)
